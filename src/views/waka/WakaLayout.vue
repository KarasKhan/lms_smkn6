...
const namaWaka = ref('Memuat...')
const nipWaka = ref('...')

onMounted(() => {
  namaWaka.value = localStorage.getItem('user_nama') || 'Waka Kurikulum'
  nipWaka.value = localStorage.getItem('user_nip') || '...'
})
...
const prosesLogout = () => {
  ...
  if (confirm('Apakah Anda yakin ingin keluar dari Portal Waka?')) {
    localStorage.clear()
    router.replace('/')
  }
}
...
<template>
  <div class="flex h-[100dvh] bg-[#F8FAFC] font-sans text-slate-800">
    <div
      v-if="menuMobileBuka"
      @click="toggleMenu"
    ...
    >
      <div class="h-20 px-6 flex items-center justify-between shrink-0 border-b border-slate-100">
        <h1 class="text-xl font-black text-purple-600 tracking-tight">
          Portal<span class="text-slate-800">Waka</span>
        </h1>
        <button
          @click="toggleMenu"
      ...
      <div class="px-4 py-6 flex-1 overflow-y-auto custom-scrollbar">
        <nav class="space-y-1.5">
          ...
          <RouterLink
            ...
            :class="[
              'flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-bold text-sm',
              route.path.includes('/waka/kelas')
                ? 'bg-purple-50 text-purple-700'
                : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800',
            ]"
          >
          ...
          <RouterLink
            ...
            :class="[
              'flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-bold text-sm',
              route.path.includes('/waka/siswa')
                ? 'bg-purple-50 text-purple-700'
                : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800',
            ]"
          >
          ...
          <RouterLink
            ...
            :class="[
              'flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-bold text-sm',
              route.path.includes('/waka/kenaikan')
                ? 'bg-purple-50 text-purple-700'
                : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800',
            ]"
          >
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden relative">
      ...
      >
        <div class="flex items-center gap-3 lg:gap-4 shrink-0">
          <button
            @click="toggleMenu"
            class="lg:hidden p-2.5 -ml-1 text-slate-500 hover:text-purple-600 transition bg-white border border-slate-200 rounded-lg shadow-sm focus:outline-none"
          >
          ...
          <button
            ...
            class="hidden lg:flex p-2.5 -ml-1 text-slate-500 hover:text-purple-600 transition bg-white border border-slate-200 rounded-lg shadow-sm focus:outline-none"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
        ...
        <div class="relative flex-1 flex justify-end ml-4">
          ...
          >
            ...
            <div
              class="w-9 h-9 sm:w-10 sm:h-10 shrink-0 bg-gradient-to-tr from-purple-600 to-fuchsia-800 rounded-full text-white flex items-center justify-center font-black shadow-md ring-2 sm:ring-4 ring-white"
            >
              {{ namaWaka.charAt(0) }}
            </div>
          </button>
          ...
          >
            <div class="px-2 space-y-1">
              <div class="px-4 py-2 border-b border-gray-100 mb-1 md:hidden">
                <p class="text-sm font-bold text-gray-800">{{ namaWaka }}</p>
              </div>
              <button
                @click="prosesLogout"
